...
function Player(num_lives)
    ...
    return {
        ...
        draw = function(self,faded)
            ...
            elseif not self.thrust.big_flame then
                     self.thrust.flame = self.thrust.flame + 1/love.timer.getFPS()
                ...
                self:draw_flame_thrust("fill",{1,0.16,0})
            end

            if debugging then 
                love.graphics.setColor(1,0,0)
                
                love.graphics.rectangle("fill",self.x-1,self.y-1,4,4)
            end
        ...
        movePlayer = function (self)
            ...
            if love.keyboard.isDown("a") or love.keyboard.isDown("left") or love.keyboard.isDown("kp4") then
                self.angle = self.angle + 0.3*self.rotation
            end
           ...
           if self.x +self.radius < 0 then 
            self.x = self.radius + love.graphics.getWidth()
           elseif self.x -self.radius > love.graphics.getWidth() then
                  self.x = -self.radius
           end

           if self.y +self.radius < 0 then 
            self.y = self.radius + love.graphics.getHeight()
